...
@UseCase(name: 'Default', type: AppLinkButton)
Widget linkButtonUseCase(BuildContext context) {
  final text = context.knobs.string(
    label: 'Text',
    initialValue: 'Link Button', // Using a generic default
  ...
  final color = context.knobs.object.dropdown<SemanticColor>(
    ...
    description: 'Defines the base color scheme for the link.',
  );
  ...
  final size = context.knobs.object.dropdown<ButtonSize>(
    ...
    labelBuilder: (value) => value.name,
    description: 'Select the size impacting text and icon scaling.',
  );

  final prefixIcon = showPrefixIcon ? Icons.link : null;
