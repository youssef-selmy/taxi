...
@Injectable()
export class EmailTemplateAuthorizer implements CustomAuthorizer<any> {
  ...
  async authorize(
    ...
    if (
      ...
      !operator.role.permissions.includes(OperatorPermission.EmailTemplates_View)
    ) {
    ...
    if (
      !authorizerContext.readonly &&
      !operator.role.permissions.includes(OperatorPermission.EmailTemplates_Edit)
    ) {
      if (
