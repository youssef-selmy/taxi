...
@Injectable()
export class EmailProviderAuthorizer implements CustomAuthorizer<any> {
  ...
  async authorize(
    ...
    if (
      ...
      !operator.role.permissions.includes(OperatorPermission.EmailProviders_View)
    ) {
    ...
    if (
      !authorizerContext.readonly &&
      !operator.role.permissions.includes(OperatorPermission.EmailProviders_Edit)
    ) {
      if (
